...
export async function POST(request: Request) {
  try {
    ...
    if (code.length < 12) return Response.json({ error: "同步码至少需要 12 位" }, { status: 400 });
    ...
    if (body.action === "pull") {
      if (!current) return Response.json({ found: false, revision: 0, data: null });
      return Response.json({ found: true, revision: current.revision, data: JSON.parse(current.payload), updatedAt: current.updatedAt });
    }

    if (body.action !== "push" || body.data === undefined) {
      return Response.json({ error: "请求内容不完整" }, { status: 400 });
    }

    const payload = JSON.stringify(body.data);
    if (payload.length > 900_000) return Response.json({ error: "同步数据过大，请先导出归档" }, { status: 413 });
    const revision = Number.isInteger(body.revision) ? Number(body.revision) : 0;
    const now = new Date().toISOString();

    if (!current) {
      if (revision !== 0) return Response.json({ conflict: true, revision: 0, data: null }, { status: 409 });
      await db.insert(syncStates).values({ codeHash, payload, revision: 1, updatedAt: now });
      return Response.json({ revision: 1, updatedAt: now });
    }

    if (current.revision !== revision) {
      return Response.json({ conflict: true, revision: current.revision, data: JSON.parse(current.payload) }, { status: 409 });
    }
    ...
    if (!result.success) return Response.json({ error: "同步写入失败" }, { status: 500 });
